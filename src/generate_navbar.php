...
function generateNavbar($session_data)
{
    ...
    if (isset($session_data['Username'])) {
        if ($session_data['isAdmin'] == 1)
            $risposta[0] = "<a href=\"./pannelloAmministrazione.php\">Pannello Amministrazione</a>";
        else
            $risposta[0] = "<a href=\"./profilo.php\">Profilo</a>";
        $risposta[1] = "
...
function generateNavbarError($session_data)
{
    $risposta = array();
    //Caso utente loggato [profilo/admin  |  Logout]
    if (isset($session_data['Username'])) {
